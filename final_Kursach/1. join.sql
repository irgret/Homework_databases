...
SELECT o.id AS '№ заказа', c.name AS 'Компания', 
...
SELECT o_status_id, os.name AS 'Тип проекта', COUNT(*) AS 'Всего в 2020' FROM orders o
JOIN o_status os ON o.o_status_id = os.id
	WHERE o_status_id IN (1, 2) 
    AND o.created_at BETWEEN '2020-01-01' AND '2020-08-01'
GROUP BY o_status_id;
...
-- итого, у нас 2 директора по продажам. Главное, что не два генеральных директора, а то был бы казус.
