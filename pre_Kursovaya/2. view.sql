...
Похоже на запрос №1 из задания про типовые выборки с тем отличием, что в данном представлении мы отсортировали данные так,
что можем видеть рядом заказы на одинаковые должности в одном статусе (например, завершен).
...
FROM orders o
	...
	JOIN o_status os
		ON o.o_status_id = os.id
	JOIN companies c
		ON o.company_id = c.id
ORDER BY o.o_status_id DESC, p.id;
...
CREATE VIEW v2 AS 
		(SELECT '1 компания' AS 'год', COUNT(*) AS column_2 FROM users_orders uo
			...
			AND o.o_status_id = (SELECT os.id FROM o_status os WHERE os.name = 'завершен')
	...
	SELECT * FROM v2;
    SELECT AVG(column_2) FROM v2;
-- то есть, в среднем мы показыали, 3,5 человека прежде, чем вакансия закрывалась.
-- Изменив в представлении выше название должности на Директор по логистике, мы узнаем средние данные и по этой вакансии.